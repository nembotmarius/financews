...
public interface PaclieApi {
    ...
    @ApiResponses(value = {
            @ApiResponse(code=200, message="Les clients1 ont été recupérés avec succès"),
            @ApiResponse(code=400, message="Echec de recuperation des clients1 de la BD"),
            @ApiResponse(code=500, message="Echec Server"),
            @ApiResponse(code=401, message="Echec d'authentification")
    })
    @ApiOperation(
            value = "Recupère tous les clients1 de la BD",
            notes = "Cette opération va recupèrer tous les clients1 dans la BD"
    )
